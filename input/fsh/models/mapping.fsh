...
* hosp.medCertBookDate -> "item.where(linkId = '1').item.where(linkId = '1.3').answer.valueDate (開立診斷書申請日期)"
* hosp.hospId -> "item.where(linkId = '1').item.where(linkId = '1.4').answer.valueCoding.code (醫事機構代碼)"
...
* diagnosis.examinationReport.reportType -> "item.where(linkId = '4').item.where(linkId = '4.2.1').item.answer.valueCoding.code (報告類型)" 
* diagnosis.examinationReport.speType -> "item.where(linkId = '4').item.where(linkId = '4.2.2').item.answer.valueString (檢體種類)" 
* diagnosis.examinationReport.reportResultString -> "item.where(linkId = '4').item.where(linkId = '4.2.3').item.answer.valueString (檢查報告結果（文數字）)" 
* diagnosis.examinationReport.reportResultPdf -> "item.where(linkId = '4').item.where(linkId = '4.2.4').item.answer.valueAttachment (檢查報告結果（檔案路徑）)" 
* diagnosis.examinationReport.reportResultPdfTitle -> "item.where(linkId = '4').item.where(linkId = '4.2.5').item.answer.valueString (檢查報告名稱)" 
* diagnosis.examinationReport.reportDate -> "item.where(linkId = '4').item.where(linkId = '4.2.6').item.answer.valueDate (報告日期)" 
...
* cancerStage.earlyStage -> "item.where(linkId = '6').item.where(linkId = '6.3').answer.valueString (初期癌症期別)" 
* cancerStage.actualStage -> "item.where(linkId = '6').item.where(linkId = '6.4').answer.valueString (實際癌症期別)" 
* cancerStage.assessDate -> "item.where(linkId = '6').item.where(linkId = '6.5').answer.valueDate (癌症分期量表評估日期)" 
